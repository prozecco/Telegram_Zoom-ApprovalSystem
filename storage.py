...
def get_db():
    """
    Context manager to handle SQLite or PostgreSQL connections and yield a cursor.
...
def execute_query(cursor, query: str, params: tuple = ()):
    """
    Executes a query, automatically adapting SQLite (?) placeholders to PostgreSQL (%s) placeholders.
    """
    if IS_POSTGRES:
