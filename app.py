...
# Helper to generate the administrative inline keyboard
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    depending on their authorization privileges.
    """
...
async def user_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Processes User menu selection callback buttons (Zoom registration link, help instructions).
    """
...
async def start_register(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """
    Entry point to user registration conversation flow via user menu button.
    """
    query = update.callback_query
    await query.answer()
...
async def submit_registration(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """
    Submits user details to the database and notifies the administrator.
    Includes the Blacklist check to flag warning without auto-rejection.
    """
    query = update.callback_query
    await query.answer()
    ...
    # Log submission to history (marked as 'Pending')
    ...
    # compile history details for admin dashboard
    history = storage.get_submissions_by_email(email)
    unique_names = set(h["submitted_zoom_name"] for h in history if h["submitted_zoom_name"])
    submission_count = len(history)
    
    if submission_count > 1:
        history_summary = (
            f"⚠️ <b>This email has applied {submission_count} times before!</b>\n"
            f"Names used: {html.escape(', '.join(unique_names))}"
        )
    else:
    ...
    if is_blacklisted:
        blacklist_warning = (
            "🚨 <b>⚠️ WARNING: THIS USER IS CURRENTLY BLACKLISTED!</b>\n"
            f"📝 <b>Behavior Notes:</b> {html.escape(behavior_notes)}\n\n"
        )
        
    # Build Admin Decision Card message
    admin_message = (
...
# ==========================================
# ADMIN DASHBOARD DECISION CALLBACK
# ==========================================

async def admin_decision_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Processes the administrator action buttons from the decision card.
    Keeps inline keyboard buttons active to support decision updates and overrides.
    """
    query = update.callback_query
    user_id = update.effective_user.id
    
    # Enforce Admin security check (dynamic)
    if not storage.is_admin(user_id):
        await query.answer("Unauthorized access.", show_alert=True)
        return
        
    data = query.data
    match = re.match(r"^(approve|deny|later|blacklist|editnotes|reviewreq)_(\d+)$", data)
    if not match:
    ...
    if not submission and action != "reviewreq":
        await query.answer("Error: Submission log not found in database.", show_alert=True)
        return
        
    email = submission["registered_email"] if submission else None
    zoom_name = submission["submitted_zoom_name"] if submission else None
    telegram_username = submission["submitted_telegram_username"] if submission else None
    telegram_id = None
    ...
    # Extract the base text of the card, removing any appended status indicators
    ...
    # Helper to notify user with error handling
    ...
    try:
        if action == "approve":
            ...
            # 1. Update Database Action to Deferred
            ...
            await try_notify_user(
                ...
                "Your Zoom registration request has been put on hold for further review. "
                "We will notify you as soon as the administrator makes a decision."
            )
            
            # 3. Update Admin Message text
            await query.edit_message_text(
            ...
            # 1. Execute Zoom Status Update (Deny user registration)
            await query.answer("Blacklisting Identity...")
            try:
                zoom_service.update_registrant_status(email, "deny")
            except Exception as e:
                logger.info("Zoom deny failed during blacklist (user might not be registered): %s", e)
                
            # 2. Update Database to Blacklisted
            ...
            await query.answer()
            # Send copy-on-tap helper text to admin
            await context.bot.send_message(
                chat_id=config.ADMIN_CHAT_ID,
                text=(
            ...
            # Admin tapped a request inline button from the /requests list
            ...
            if not submission:
                await context.bot.send_message(
                    chat_id=user_id,
                    text="⚠️ Error: Submission record not found."
                )
                return
            ...
            user_record = storage.get_user_by_email(email)
            telegram_id = user_record["telegram_id"] if user_record else "None"
            global_status = user_record["global_status"] if user_record else "Pending"
            behavior_notes = user_record["behavior_notes"] if user_record else ""
        ...
        # Prepend the error message but keep the buttons active so they can try again
        await query.edit_message_text(
...
async def show_config_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """
    Displays the current Zoom variables configurations with action buttons.
    """
...
async def admin_manage_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """
    Starts the Admin Rights Management conversation, listing current admins and actions.
    """
    user_id = update.effective_user.id
    if not storage.is_admin(user_id):
...
async def admin_manage_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """
    Handles Add, Remove, and Back choices on the Admin rights panel.
    """
    query = update.callback_query
    await query.answer()
...
async def add_admin_input_received(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    storage.add_admin(new_admin_id, username=f"Admin_{new_admin_id}")
    
    await update.message.reply_text(f"✅ Successfully authorized Telegram ID <code>{new_admin_id}</code> as an administrator!", parse_mode="HTML")
    return await admin_manage_start(update, context)
...
async def cancel_admin_manage(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await reply_helper(update, "🛡️ <b>Admin Control Panel</b>\n\nAdmin management closed. Select a task below:", reply_markup=reply_markup)
    return ConversationHandler.END
...
async def admin_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Routes callback queries from the Admin Control Panel main menu and back buttons.
    """
    query = update.callback_query
    await query.answer()
...
async def synczoom_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Retrieves all registrants for the active meeting from Zoom and synchronizes
    ...
    try:
        # Loop through pending, approved, and denied registrants on Zoom
        for zoom_status, db_status in [
            ...
            for r in registrants:
                ...
                # Also log to submissions history if empty
                history = storage.get_submissions_by_email(email)
                if not history:
...
async def blacklist_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    Usage: /blacklist <email> [optional_notes]
    """
    if not storage.is_admin(update.effective_chat.id):
        await update.message.reply_text("Unauthorized access.")
        return
...
async def requests_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Lists all registration requests in the database, grouped by their read/unread status.
    Provides interactive inline keyboard buttons to view/process each request directly.
    """
    if not storage.is_admin(update.effective_chat.id):
    ...
    with storage.get_db() as conn:
        cursor = conn.execute(
            """
            SELECT u.registered_email, u.global_status, MAX(s.submitted_zoom_name) as zoom_name
            FROM users u
            LEFT JOIN submissions_history s ON u.registered_email = s.registered_email
            GROUP BY u.registered_email
    ...
    for row in rows:
        ...
        # Check the latest action in history
        with storage.get_db() as conn2:
            c2 = conn2.execute(
                "SELECT id, action_taken FROM submissions_history WHERE registered_email = ? ORDER BY action_timestamp DESC LIMIT 1", 
                (email,)
            )
            latest = c2.fetchone()
            sub_id = latest["id"] if latest else 0
            latest_action_str = latest["action_taken"] if latest else None
            
        if status == "Pending":
            if latest_action_str == "Deferred":
                deferred_list.append((name, email, sub_id))
            else:
                unread_list.append((name, email, sub_id))
        else:
            processed_list.append((name, email, sub_id, status))
            
    message = "📋 <b>Registration Request List (Read/Unread Status):</b>\n\n"
    keyboard = []
    
    if unread_list:
        message += "📥 <b>Unread / Unprocessed (Pending):</b>\n"
        for name, email, sub_id in unread_list:
            message += f"- 🟡 {html.escape(name)} (<code>{html.escape(email)}</code>)\n"
            keyboard.append([InlineKeyboardButton(f"🟡 Review: {name}", callback_data=f"reviewreq_{sub_id}")])
        message += "\n"
        
    if deferred_list:
        message += "⏳ <b>Deferred / Review Later:</b>\n"
        for name, email, sub_id in deferred_list:
            message += f"- ⏳ {html.escape(name)} (<code>{html.escape(email)}</code>)\n"
            keyboard.append([InlineKeyboardButton(f"⏳ Review: {name}", callback_data=f"reviewreq_{sub_id}")])
        message += "\n"
        
    if processed_list:
        message += "✅ <b>Read / Processed Profiles:</b>\n"
        for name, email, sub_id, status in processed_list:
            status_emoji = "🟢" if status == "Approved" else ("🔴" if status == "Denied" else "🚫")
            message += f"- {status_emoji} {html.escape(name)} (<code>{html.escape(email)}</code>) [<i>{status}</i>]\n"
            keyboard.append([InlineKeyboardButton(f"{status_emoji} Review: {name}", callback_data=f"reviewreq_{sub_id}")])
        message += "\n"
        
    if not unread_list and not deferred_list and not processed_list:
        message = "📋 <b>Registration Request List:</b>\n\n<i>No profiles found in the database.</i>"
...
async def review_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not history:
        ...
        history_summary = "✨ No historical submissions."
    else:
        latest = history[0]
        sub_id = latest["id"]
        zoom_name = latest["submitted_zoom_name"]
...
def main() -> None:
    ...
    storage.init_db()
    logger.info("SQLite relational database initialized successfully.")

    # 1.5 Start health check server for Hugging Face or other PaaS platforms
    threading.Thread(target=start_health_check_server, daemon=True).start()

    # 2. Build the Application with custom HTTPX timeouts to prevent slow connection handshakes
    ...
    application.add_handler(
        CallbackQueryHandler(admin_decision_callback, pattern="^(approve|deny|later|blacklist|editnotes|reviewreq)_\\d+$")
    )
    
    # 4.5 Add Main Menu Navigation Callbacks
    application.add_handler(
        CallbackQueryHandler(admin_menu_callback, pattern="^(admin_requests|admin_config|admin_report|back_to_admin_panel)$")
    )
    application.add_handler(
